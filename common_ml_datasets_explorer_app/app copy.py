...
# Requests config
config = {
    "data_gathering_url_endpoint": 'https://data_gathering_services:5704',
    "csrf_token_key":"gTiqBYe4fn2P8QzsVEiCrTNb14bhK7xakHR2UaublauCZmzcfuuN4hgINto4MpiS"
}
	...
	if choice == "Pandas Profile":
		...
		if data_file is not None:
			...
			if st.button("Generate Sweetviz Report"):
				...
				table_name = st.text_input("Table Name", "table name")
				database_name = st.text_input("Server", "Server name")
				server_name = st.text_input("Database", "database name")
				if st.button("Save Data to Database"):
					while response == {}:
						response = requests.post(
						url=str(config["data_gathering_url_endpoint"]) + "/data_gathering/write/to_sql/from_profiling",
						data={
							"file": df.to_json(),
					...
					st.subheader(response.json()["output"])
